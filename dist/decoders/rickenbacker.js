...
function decodeTwoLetter(serial) {
...
function decodeReversed(serial) {
...
function decodeTransitional(serial) {
    const monthLetter = serial.charAt(0);
    const yearDigit = serial.charAt(1);
    const sequence = serial.substring(2);
...
function decodeEarlySolidbody(serial) {
    // Parse model prefix
    let model;
    let remaining;
...
// Short scale format (1954-1959): V + Year + Sequence
function decodeShortScale(serial) {
    const yearDigit = serial.charAt(1);
    const sequence = serial.substring(2);
    const year = 1950 + parseInt(yearDigit, 10);
    const info = {
...
// Sequence only (could be 1959-1960 gap period)
function decodeSequenceOnly(serial) {
