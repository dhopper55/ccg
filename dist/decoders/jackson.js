...
function decodeImportIShort(serial) {
...
function decodeModernJIndonesia(serial) {
    const yearDigits = serial.substring(1, 3);
    const sequence = serial.substring(3);
...
function decodeJapanMijSevenPrefixSixDigit(serial) {
...
function decodeModernIndonesiaKorea8Digit(serial) {
    const yearDigits = serial.substring(0, 2);
...
function decodeEarlyBudgetNineDigit(serial) {
    const yearDigits = serial.substring(0, 2);
    const sequence = serial.substring(2);
    const yearNum = parseInt(yearDigits, 10);
    const year = (yearNum < 50 ? 2000 + yearNum : 1900 + yearNum).toString();
...
function decodeModern11DigitNumeric(serial) {
    const yearDigits = serial.substring(0, 2);
