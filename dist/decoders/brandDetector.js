 ...
 */
export function detectBrand(serial) {
...
function isFenderPattern(serial) {
    // US prefix (2010+)
    if (/^US\d{8,}$/.test(serial))
        return true;
    ...
    if (/^N\d{5,}$/.test(serial))
        return true;
    // E prefix (1980s) - distinctive for Fender
    if (/^E\d{5,}$/.test(serial))
        return true;
    // S prefix (1970s)
    if (/^S\d{5,}$/.test(serial))
        return true;
...
function isEpiphonePattern(serial) {
    ...
    if (/^\d{9,12}$/.test(serial)) {
        ...
        if (month >= 1 && month <= 12 && validFactoryCodes.includes(factoryCode))
            return true;
    }
    return false;
}
function isGibsonPattern(serial) {
...
function isTaylorPattern(serial) {
...
function isMartinPattern(serial) {
