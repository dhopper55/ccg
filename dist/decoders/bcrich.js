...
function decodeImportLetterPrefixShort(serial) {
    const prefix = serial[0];
    const yearDigits = serial.slice(1, 3);
    const yearNum = parseInt(yearDigits, 10);
    const production = serial.slice(3);
...
function decodeHanserSingleLetterCalendarMonthImport(serial) {
    const monthCode = serial[0];
...
function decodeNineDigitImport(serial) {
...
function decodeClassAxeCA(serial) {
    const sequence = serial.slice(2);
    const sequenceNumber = parseInt(sequence, 10);
    const info = {
        brand: 'B.C. Rich',
...
function decode10DigitNumericImport(serial) {
    const info = {
        brand: 'B.C. Rich',
        serialNumber: serial,
...
function decodeBWPrefixImport(serial) {
...
function decodeEarlyUSA4Digit(serial) {
