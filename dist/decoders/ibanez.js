...
function decodeKnownProductCode(productCode) {
    const info = {
        brand: 'Ibanez',
...
function decodeJapan1987to1996(serial) {
...
function decodeJapan1975to1988(serial) {
...
function decodeH1994to1998(serial) {
    const year = parseInt(serial.substring(1, 3), 10);
    const sequence = serial.substring(3);
...
// Japan/Korea hybrid: FC + 7 digits
function decodeFCHybrid(serial) {
...
function decodeKoreaSaeinSAPrefix(serial) {
...
function decodeKorea2000to2008(serial) {
    const factoryCode = serial[0];
...
function decodeKorea1995to1999(serial) {
    const factoryCode = serial[0];
    const yearDigit = parseInt(serial[1], 10);
...
function decodeKorea1987to1995(serial) {
    const factoryCode = serial[0];
    const yearDigit = parseInt(serial[1], 10);
...
function decodeKoreaCort1990s7Digit(serial) {
    const factoryCode = serial[0];
    const yearDigit = parseInt(serial[1], 10);
    const month = parseInt(serial.substring(2, 4), 10);
    const sequence = serial.substring(4);
...
function decodeWorldExtended(serial) {
    const yearDigit = parseInt(serial[1], 10);
    const month = parseInt(serial.substring(2, 4), 10);
    const sequence = serial.substring(4);
    const year = yearDigit === 9 ? 1999 : 2000 + yearDigit;
...
function decodeSaehanMonthLetter(serial) {
...
// Import two-character prefix variant: 5A/5B/5N + 9 digits
function decodeTwoCharImportPrefix9Digit(serial) {
...
function decodeNumeric10DigitFactoryLeading(serial) {
...
function decodeIndonesiaPremiumJYearSequence(serial) {
    const yearDigits = serial.substring(1, 3);
    const sequence = serial.substring(3);
    const year = 2000 + parseInt(yearDigits, 10);
    const sequenceNumber = parseInt(sequence, 10);
...
function decodeIndonesia1997to2000(serial) {
    const yearDigit = parseInt(serial[1], 10);
    const month = parseInt(serial.substring(2, 4), 10);
    const sequence = serial.substring(4);
...
// Indonesia GIO legacy: GI + 7 digits
function decodeGioIndonesiaLegacy(serial) {
    const year = parseInt(serial.substring(2, 4), 10) + 2000;
    const month = parseInt(serial.substring(4, 6), 10);
    const sequence = serial.substring(6);
    const info = {
...
function decodeIndonesiaPremium(serial) {
    const monthLetter = serial[0];
...
function decodeCompactAlphaSuffix(serial) {
    const yy = parseInt(serial.substring(0, 2), 10);
...
function decodeAmbiguousSixDigitNumericWithImpossibleYear(serial, yy) {
    const sequence = serial.substring(2);
...
function decodeJCustom5Digit(serial) {
    const yearDigit = parseInt(serial[0], 10);
...
function getKoreanFactory(code) {
    switch (code) {
        case 'C':
