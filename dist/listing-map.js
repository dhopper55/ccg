...
const mapEl = document.getElementById('listing-map-canvas');
const statusEl = document.getElementById('listing-map-status');
const countsEl = document.getElementById('listing-map-counts');
const apiKeyInput = document.getElementById('listing-map-api-key');
const loadButton = document.getElementById('listing-map-load');
const clearKeyButton = document.getElementById('listing-map-clear-key');
...
function setLoading(isLoading) {
    ...
    if (clearKeyButton)
        clearKeyButton.disabled = isLoading;
    if (apiKeyInput)
...
async function loadGoogleMapsApi(apiKey) {
    if (window.google && typeof google?.maps === 'object')
        return;
...
async function geocodeLocation(geocoder, location) {
...
function offsetPosition(base, index) {
    if (index === 0)
        return base;
...
async function renderMapWithListings() {
    if (!mapEl)
        return;
    ...
    if (candidates.length === 0) {
        if (countsEl)
            countsEl.textContent = 'No non-archived listings with location.';
