...
let reverbQueriesModalEl: HTMLDialogElement | HTMLDivElement | null = null;
...
function openDoubleCheckQuery(query: string): void {
  if (!query) return;
...
function parseReverbQueriesFromText(value: unknown): string[] {
...
function getReverbQueries(fields: Record<string, unknown>): string[] {
...
function buildReverbSearchUrl(query: string): string {
...
function buildGoogleReverbSoldFallbackUrl(query: string): string {
...
function closeReverbQueriesModal(): void {
  ...
  if (reverbQueriesModalEl instanceof HTMLDialogElement) {
    if (reverbQueriesModalEl.open) reverbQueriesModalEl.close();
    return;
  }
  reverbQueriesModalEl.classList.add('hidden');
}

function ensureReverbQueriesModal(): HTMLDialogElement | HTMLDivElement {
  if (reverbQueriesModalEl) return reverbQueriesModalEl;
  ...
  if (typeof HTMLDialogElement !== 'undefined') {
    ...
    dialog.addEventListener('click', (event) => {
      ...
      if (target?.dataset.close != null) closeReverbQueriesModal();
    });
  ...
  overlay.addEventListener('click', (event) => {
    const target = event.target as HTMLElement | null;
    if (target === overlay || target?.dataset.close != null) closeReverbQueriesModal();
  });
...
function openReverbQueriesModal(queries: string[]): void {
  if (!queries.length) return;
  const modal = ensureReverbQueriesModal();
  const body = modal.querySelector('[data-body]') as HTMLDivElement | null;
  if (!body) return;
  body.innerHTML = '';
...
function buildReverbQueriesLauncher(queries: string[]): HTMLElement {
...
function formatMultiSummary(text: string, fields: Record<string, unknown>): DocumentFragment | null {
  ...
  const rows: Array<{ text: string; query?: string }> = [];
  let askingSum = 0;
  let lowSum = 0;
  let highSum = 0;
  let idealSum = 0;
...
function addMetaNodeRow(label: string, content: Node | string): void {
  if (!metaEl) return;
  const term = document.createElement('dt');
  term.textContent = label;
  const detail = document.createElement('dd');
...
function addMetaIconRow(label: string, value: string | undefined): void {
  if (!metaEl) return;
  const term = document.createElement('dt');
  term.textContent = label;
  const detail = document.createElement('dd');
...
function renderRecord(record: ListingRecordResponse): void {
  ...
  if (thumbnailEl && mediaEl) {
    ...
    const hasImage = setThumbnailWithFallback(
      thumbnailEl,
      photoCandidates,
      title,
