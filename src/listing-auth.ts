...
function registerAdminPwa(): void {
  if (adminPwaRegistrationAttempted) return;
...
export function initListingAuth() {
  ...
  let overlay = document.getElementById('listing-auth') as HTMLDivElement | null;
  if (!overlay) {
    overlay = buildOverlay();
  }

  const form = overlay.querySelector<HTMLFormElement>('#listing-auth-form');
  const userInput = overlay.querySelector<HTMLInputElement>('#listing-auth-user');
  const passInput = overlay.querySelector<HTMLInputElement>('#listing-auth-pass');
  const errorEl = overlay.querySelector<HTMLDivElement>('#listing-auth-error');
