...
const JAPAN_LETTER_YEARS: Record<string, { years: string; factory: string }> = {
...
function decodeJapanJV(serial: string): DecodeResult {
...
function decodeJapanSQ(serial: string): DecodeResult {
  const info: GuitarInfo = {
    brand: 'Squier',
    serialNumber: serial,
    year: '1983-1984',
...
function decodeEPrefix(serial: string): DecodeResult {
...
function decodeJapanLetter(letter: string, sequence: string, serial: string): DecodeResult {
...
function decodeKoreaKC(serial: string): DecodeResult {
...
function decodeKoreaKV(serial: string): DecodeResult {
  const yearDigits = serial.substring(2, 4);
  let year = parseInt(yearDigits, 10);
  year = year >= 90 ? 1900 + year : 2000 + year;
...
function decodeKoreaCN(serial: string): DecodeResult {
  const yearDigit = serial[2];
...
function decodeKoreaS(serial: string): DecodeResult {
  const yearDigit = serial[1];
...
function decodeKoreaM(serial: string): DecodeResult {
  const yearDigit = serial[1];
...
function decodeKoreaNumeric(serial: string): DecodeResult {
...
function decodeChinaSquierSE10Digit(serial: string): DecodeResult {
...
function decodeIndonesiaICS(monthLetter: string, yearDigits: string, sequence: string, serial: string): DecodeResult {
...
function decodeIndonesiaICSYear(yearDigits: string, sequence: string, serial: string): DecodeResult {
  const year = 2000 + parseInt(yearDigits, 10);
...
function decodeIndonesiaIC(yearDigits: string, sequence: string, serial: string): DecodeResult {
  const year = 2000 + parseInt(yearDigits, 10);
...
function decodeIndonesiaISSLetter(monthLetter: string, yearDigits: string, sequence: string, serial: string): DecodeResult {
  const year = 2000 + parseInt(yearDigits, 10);
  const month = MONTH_LETTERS[monthLetter] || 'Unknown';
  const sequenceNumber = parseInt(sequence, 10);
...
function decodeIndonesiaISS(serial: string): DecodeResult {
  const info: GuitarInfo = {
    brand: 'Squier',
    serialNumber: serial,
    factory: 'Samick',
...
function decodeIndonesiaIS(yearDigits: string, sequence: string, serial: string): DecodeResult {
  const year = 2000 + parseInt(yearDigits, 10);
...
function decodeIndonesiaSI(yearDigits: string, sequence: string, serial: string): DecodeResult {
  const year = 2000 + parseInt(yearDigits, 10);
...
function decodeChinaCMC(monthLetter: string, yearDigits: string, sequence: string, serial: string): DecodeResult {
  const year = 2000 + parseInt(yearDigits, 10);
  const month = MONTH_LETTERS[monthLetter] || 'Unknown';
  const sequenceNumber = parseInt(sequence, 10);
...
function decodeChinaCSK(serial: string): DecodeResult {
...
function decodeChinaCSV(monthLetter: string, yearDigits: string, sequence: string, serial: string): DecodeResult {
  const year = 2000 + parseInt(yearDigits, 10);
  const month = MONTH_LETTERS[monthLetter] || 'Unknown';
  const sequenceNumber = parseInt(sequence, 10);
...
function decodeChinaYN(serial: string): DecodeResult {
  const yearDigit = serial[2];
...
function decodeChinaCXS(monthDigits: string, yearDigits: string, sequence: string, serial: string): DecodeResult {
  const year = 2000 + parseInt(yearDigits, 10);
...
function decodeChinaCA(yearDigits: string, sequence: string, serial: string): DecodeResult {
  const year = 2000 + parseInt(yearDigits, 10);
...
function decodeChinaCG(yearDigits: string, sequence: string, serial: string): DecodeResult {
  const year = 2000 + parseInt(yearDigits, 10);
...
function decodeChinaCO(yearDigits: string, sequence: string, serial: string): DecodeResult {
  const year = 2000 + parseInt(yearDigits, 10);
...
function decodeChinaCSSLetterMonth(monthLetter: string, yearDigits: string, sequence: string, serial: string): DecodeResult {
  const MONTH_LETTERS: Record<string, string> = {
...
function decodeChinaCSS(yearDigits: string, sequence: string, serial: string): DecodeResult {
  const year = 2000 + parseInt(yearDigits, 10);
...
function decodeChinaNC(serial: string): DecodeResult {
  const yearDigit = serial[2];
...
function decodeMexicoMN(yearDigit: string, sequence: string, serial: string): DecodeResult {
  const year = `199${yearDigit}`;
...
function decodeMexicoMZ(yearDigit: string, sequence: string, serial: string): DecodeResult {
...
function decodeUSAN(serial: string): DecodeResult {
  const yearDigit = serial[1];
