...
function decodeFujiGenPost2004EightDigit(serial: string): DecodeResult {
  const year = parseInt(serial.substring(1, 3), 10);
  const productionNum = parseInt(serial.substring(3), 10);
...
// Legacy Korean M prefix: M + YY + 4-digit sequence
function decodeLegacyKoreaMPrefix(serial: string): DecodeResult {
  const yearDigits = serial.substring(1, 3);
  const sequence = serial.substring(3);
...
function decodeChinaGaoqingGrandStar(serial: string): DecodeResult {
  const yearDigits = serial.substring(1, 3);
...
function decodeChinaGSMixedContractor(serial: string): DecodeResult {
  const yearDigits = serial.substring(2, 4);
