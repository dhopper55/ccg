...
function decodeIndonesiaYearFactorySequence(serial: string): DecodeResult {
...
// Chinese: shorter G + 5-digit variant (G + YY + 3-digit sequence, no month)
function decodeChinaShort(serial: string): DecodeResult {
  const digits = serial.substring(1);
...
function decodeModernTwoCharFactoryYYMMSeq(serial: string): DecodeResult {
...
// e.g., 0C060872 = 0C factory, 2006, sequence 0872
function decodeModernTwoCharFactoryYYSeq(serial: string): DecodeResult {
  const prefix = serial.substring(0, 2);
  const yearDigits = serial.substring(2, 4);
...
function decodeTwoCharFactoryYYSequenceShort(serial: string): DecodeResult {
  const prefix = serial.substring(0, 2);
  const yearDigits = serial.substring(2, 4);
  const sequence = serial.substring(4);
  const yearNum = parseInt(yearDigits, 10);
...
function decodeCOVariantFactory(serial: string): DecodeResult {
  const prefixMatch = serial.match(/^(OCO|CO|O)(\d)(\d{6})$/);
  if (!prefixMatch) return { success: false, error: 'Unable to decode CO-variant factory serial.' };
...
function decodeLate80s90sNumeric10Digit(serial: string): DecodeResult {
...
// GO factory prefix: GO + single-digit year + 5-7 digit sequence
function decodeGOPrefixFactory(serial: string): DecodeResult {
  const digits = serial.substring(2);
...
function decodeVintageYamakiJapan(serial: string): DecodeResult {
  ...
  const YAMAKI_SUFFIX_MAP: Record<string, string> = {
    E: 'Yamaki (Japan)',
...
function getSingleDigitImportYears(yearDigit: number): string {
  if (Number.isNaN(yearDigit)) return 'Unknown';
