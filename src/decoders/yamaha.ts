...
export function decodeYamaha(serial: string): DecodeResult {
...
function getMonthFromLetter(letter: string): number {
  const mapping: Record<string, number> = {
    'H': 1,  // January
    'I': 2,  // February
    'J': 3,  // March
...
function getPossibleYears(yearDigit: number): string {
  const baseYears = [];
...
// Standard format: Letter-Letter-##-### (e.g., HM02316)
function decodeStandard(serial: string): DecodeResult {
...
function decodeStandardExtended(serial: string): DecodeResult {
  const yearLetter = serial[0];
  const monthLetter = serial[1];
  const day = parseInt(serial.substring(2, 4), 10);
  const unit = parseInt(serial.substring(4), 10);
...
function decodeThreeLetterYearExtended(serial: string): DecodeResult {
...
function decodeJapanCustomShop2004(serial: string): DecodeResult {
  const yearLetter1 = serial[0];
  const yearLetter2 = serial[1];
  const monthLetter = serial[2];
...
function decodeJapanCustomShop1997(serial: string): DecodeResult {
  const yearLetter = serial[0];
  const monthLetter = serial[1];
...
function decodeJapanCustomShop1991(serial: string): DecodeResult {
  const yearLetter = serial[0];
  const monthLetter = serial[1];
...
function decodeJapanElectricExtended(serial: string): DecodeResult {
...
function decodeJapanElectric1997(serial: string): DecodeResult {
  const yearDigit = parseInt(serial[0], 10);
  const factoryLetter = serial[1];
  const monthLetter = serial[2];
...
function decodeJapanElectric1994(serial: string): DecodeResult {
  const yearDigit = parseInt(serial[0], 10);
  const factoryLetter = serial[1];
  const monthLetter = serial[2];
  const unit = parseInt(serial.substring(3), 10);
...
function decodeJapanElectric1989(serial: string): DecodeResult {
...
function decodeJapanElectric1986(serial: string): DecodeResult {
  const yearDigit = parseInt(serial[0], 10);
  const monthLetter = serial[1];
...
function decodeJapanElectric1984(serial: string): DecodeResult {
  const yearLetter = serial[0];
  const monthLetter = serial[1];
  const unit = parseInt(serial.substring(2), 10);
...
function decodeTenryu1969(serial: string): DecodeResult {
...
function decodeTenryu1946(serial: string): DecodeResult {
...
function decodeTaiwan2002(serial: string): DecodeResult {
  const yearLetter1 = serial[0];
  const yearLetter2 = serial[1];
  const monthLetter = serial[2];
  const day = parseInt(serial.substring(3, 5), 10);
...
// Indonesia 2000+: 10 digits YYMMDDXXXX
function decodeIndonesia2000(serial: string): DecodeResult {
...
// Indonesia 1997-1999: 9 digits YMMDDXXXX
function decodeIndonesia1997(serial: string): DecodeResult {
  const yearDigit = parseInt(serial[0], 10);
...
// Indonesia 1990-1996: 8 digits YMMDDXXX
function decodeVintage7DigitSequential(serial: string): DecodeResult {
...
function decodeIndonesia1990(serial: string): DecodeResult {
  const yearDigit = parseInt(serial[0], 10);
  const month = parseInt(serial.substring(1, 3), 10);
  const day = parseInt(serial.substring(3, 5), 10);
  const unit = parseInt(serial.substring(5), 10);
...
function decodeKoreaChina2003(serial: string): DecodeResult {
  const yearLetter1 = serial[0];
  const yearLetter2 = serial[1];
  const monthLetter = serial[2];
