...
export function decodeAlvarez(serial: string): DecodeResult {
...
function decodeEmperorCode(serial: string): DecodeResult {
...
function decodeYairiSequential(serial: string): DecodeResult {
...
function decodeEmperorWithSequence(serial: string): DecodeResult {
  const emperorYear = parseInt(serial.substring(0, 2), 10);
  const month = parseInt(serial.substring(2, 4), 10);
...
function decodeNumeric8Digit(serial: string): DecodeResult {
...
function convertEmperorYear(emperorYear: number): { year: string; notes: string } {
...
function getMonthName(month: number): string | undefined {
  const months = [
    'January', 'February', 'March', 'April', 'May', 'June',
    'July', 'August', 'September', 'October', 'November', 'December',
  ];
