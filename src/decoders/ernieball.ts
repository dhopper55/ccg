...
export function decodeErnieBall(serial: string): DecodeResult {
...
// H prefix: Current production (2021+)
function decodeHSeries(serial: string): DecodeResult {
  const sequence = serial.substring(1);
...
// G prefix: Standard production (1998-2021)
function decodeGSeries(serial: string): DecodeResult {
  const sequence = serial.substring(1);
...
// D prefix: Ball Family Reserve (2021+)
function decodeDSeries(serial: string): DecodeResult {
  const sequence = serial.substring(1);
...
// L prefix: Left-handed instruments
function decodeLSeries(serial: string): DecodeResult {
  const sequence = serial.substring(1);
...
// S prefix: Artist signature models
function decodeSSeries(serial: string): DecodeResult {
  const sequence = serial.substring(1);
...
// X prefix: SUB-1 guitars (2003-2007)
function decodeXSeries(serial: string): DecodeResult {
  const sequence = serial.substring(1);
...
// A prefix: Transitional period (1990s)
function decodeASeries(serial: string): DecodeResult {
...
// E prefix: Early EB production
function decodeESeries(serial: string): DecodeResult {
  const digits = serial.substring(1);
...
// B prefix: Pre-EB and early EB basses
function decodeBSeries(serial: string): DecodeResult {
  const digits = serial.substring(1);
...
// Sterling by Music Man: SR prefix (Indonesia)
function decodeSterling(serial: string): DecodeResult {
