...
function decodeModernNumericWithSuffix(normalized: string, cleaned: string): DecodeResult {
...
function decodeGibsonEraJKIndonesia(normalized: string, cleaned: string): DecodeResult {
  const yearPart = normalized.substring(2, 4);
  const monthPart = normalized.substring(4, 6);
  const sequence = normalized.substring(6);
  const yy = parseInt(yearPart, 10);
...
function decodeShortSPlateSerial(normalized: string, cleaned: string): DecodeResult {
  const sequence = normalized.substring(1);
  const sequenceNumber = parseInt(sequence, 10);
...
function decodeUnsungKoreaU(normalized: string, cleaned: string): DecodeResult {
