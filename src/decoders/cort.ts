...
export function decodeCort(serial: string): DecodeResult {
...
function decodeIndonesiaAI(serial: string): DecodeResult {
  const yearDigits = serial.substring(2, 4);
  const monthDigits = serial.substring(4, 6);
  const sequence = serial.substring(6);
...
function decodeIndonesiaIIA(serial: string): DecodeResult {
  const yearDigits = serial.substring(3, 5);
  const monthDigits = serial.substring(5, 7);
  const sequence = serial.substring(7);
...
function decodeIndonesiaIA(serial: string): DecodeResult {
  const yearDigits = serial.substring(2, 4);
  const monthDigits = serial.substring(4, 6);
  const sequence = serial.substring(6);
  const year = 2000 + parseInt(yearDigits, 10);
...
const CORT_MONTH_LETTERS: Record<string, string> = {
...
function decodeWMIKoreaMonthLetter(serial: string): DecodeResult {
...
function decodeIndonesiaICS(serial: string): DecodeResult {
  const yearDigits = serial.substring(3, 5);
  const year = 2000 + parseInt(yearDigits, 10);
  const sequence = serial.substring(5);
...
function decodeIndonesiaICSE(serial: string): DecodeResult {
...
function decodeIndonesiaICF(serial: string): DecodeResult {
  const yearDigits = serial.substring(3, 5);
  const year = 2000 + parseInt(yearDigits, 10);
  const sequence = serial.substring(5);
...
function decodeRPrefixYearSequence(serial: string): DecodeResult {
  const yearDigits = serial.substring(1, 3);
...
function decodeIndonesiaFPrefix(serial: string): DecodeResult {
  const yearDigits = serial.substring(1, 3);
  const sequence = serial.substring(3);
  const year = 2000 + parseInt(yearDigits, 10);
...
function decodeIndonesiaIE(serial: string): DecodeResult {
  const yearDigits = serial.substring(2, 4);
  const monthDigits = serial.substring(4, 6);
  const sequence = serial.substring(6);
...
// Chinese COB prefix
function decodeChinaCOB(serial: string): DecodeResult {
  const yearDigits = serial.substring(3, 5);
  const year = 2000 + parseInt(yearDigits, 10);
  const sequence = serial.substring(5);
...
function decodeModern7DigitYearBatch(serial: string): DecodeResult {
...
function decodeModern9Digit(serial: string): DecodeResult {
  const yearDigits = serial.substring(0, 2);
...
function decodeModern10Digit(serial: string): DecodeResult {
  const yearDigits = serial.substring(0, 2);
  const monthDigits = serial.substring(2, 4);
  const sequence = serial.substring(4);
...
function decodeModern8Digit(serial: string): DecodeResult {
  const yearDigits = serial.substring(0, 2);
  const monthDigits = serial.substring(2, 4);
  const sequence = serial.substring(4);
...
function decodeYearSequence7Digit(serial: string): DecodeResult {
  const yearDigits = serial.substring(0, 2);
  const sequence = serial.substring(2);
  const year = 2000 + parseInt(yearDigits, 10);
...
function decodeEarly1990s7DigitYearSequence(serial: string): DecodeResult {
  const yearDigits = serial.substring(0, 2);
  const sequence = serial.substring(2);
  const year = 1900 + parseInt(yearDigits, 10);
  const sequenceNumber = parseInt(sequence, 10);
...
function decode1990s7Digit(serial: string): DecodeResult {
...
function decodeMGMSignatureSeries(serial: string): DecodeResult {
  const sequence = serial.substring(3);
  const sequenceNumber = parseInt(sequence, 10);
...
function decodeCortUSPrefix(serial: string): DecodeResult {
  const yearDigits = serial.substring(2, 4);
  const monthDigits = serial.substring(4, 6);
  const sequence = serial.substring(6);
  const year = 2000 + parseInt(yearDigits, 10);
