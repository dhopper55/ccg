...
export function decodeGuild(serial: string): DecodeResult {
...
function getYearFromLetter(letter: string): number | null {
...
function julianToDate(julian: number): { month: string; day: number } | null {
...
function decodeTacoma(serial: string): DecodeResult {
...
function decodeNewHartford(serial: string): DecodeResult {
  const yearLetter = serial.charAt(1);
  const julianStr = serial.substring(2, 5);
  const unitNum = serial.substring(5);
...
// Corona production: C prefix
function decodeCorona(serial: string): DecodeResult {
...
// Korean production: KS prefix
function decodeKoreaKS(serial: string): DecodeResult {
...
function decodeEEPrefixSlimJim(serial: string): DecodeResult {
...
function decodeWesterlyG3(serial: string): DecodeResult {
  const digits = serial.substring(2);
  const yearDigits = digits.substring(0, 2);
  const sequence = digits.substring(2);
  const year = 2000 + parseInt(yearDigits, 10);
...
// Westerly Collection, sibling contracted factory code: G2 prefix + YY + 5-digit sequence
function decodeWesterlyG2(serial: string): DecodeResult {
  const digits = serial.substring(2);
  const yearDigits = digits.substring(0, 2);
  const sequence = digits.substring(2);
  const year = 2000 + parseInt(yearDigits, 10);
...
// GAD series
function decodeGAD(serial: string): DecodeResult {
...
function decodeSequential(serial: string): DecodeResult {
...
function decodeLongNumeric(serial: string): DecodeResult {
  // Try to extract year from first digits
