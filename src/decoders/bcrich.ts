...
// WMI factory, alternate layout: W + month-letter(A-P) + YY(2) + batch(2) + seq(4)
function decodeWMIFactoryMonthFirst(serial: string): DecodeResult {
...
function decodeJapanTPrefixImport(serial: string): DecodeResult {
...
function decodeWMIIndonesiaWI(serial: string): DecodeResult {
  const yearDigits = serial.substring(2, 4);
  const monthDigits = serial.substring(4, 6);
  const sequence = serial.substring(6);
  const yearNum = parseInt(yearDigits, 10);
