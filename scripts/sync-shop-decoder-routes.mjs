...
const shopOutputRoot = path.join(repoRoot, 'guitars-and-gear-for-sale');
const sourceHtml = path.join(shopOutputRoot, 'index.html');
...
const routeConfigs = [
  {
    ...
    routePath: '/decoders/guitar-serial-decoder-lookup/',
...
if (!fs.existsSync(sourceHtml)) {
  throw new Error(`Missing built shop-app entry: ${sourceHtml}`);
}

const buildStamp = Date.now().toString();
const rawHtml = fs.readFileSync(sourceHtml, 'utf8');
const stampedHtml = rawHtml.replace(
  /(\/guitars-and-gear-for-sale\/assets\/[^"'?]+\.(?:js|css))(?!\?v=)/g,
  `$1?v=${buildStamp}`,
);
...
function injectRouteSeo(baseHtml, config) {
  ...
  html = html
    .replace(/\s*<link\s+rel="canonical"[\s\S]*?\/>\s*/g, '\n')
    .replace(/\s*<meta\s+property="og:[^"]+"[\s\S]*?\/>\s*/g, '\n')
    .replace(/\s*<meta\s+name="twitter:[^"]+"[\s\S]*?\/>\s*/g, '\n');
...
for (const routeConfig of routeConfigs) {
  ...
  const targetDir = path.join(repoRoot, routeConfig.routeDir);
  fs.mkdirSync(targetDir, { recursive: true });
  fs.writeFileSync(path.join(targetDir, 'index.html'), routeHtml);
}

console.log(`Synced ${routeConfigs.length} shop decoder route entries with build stamp ${buildStamp}.`);
