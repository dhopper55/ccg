...
- `ccg_inventory_items.sales_tax_included` marks items whose listed/sale price already includes sales tax (no separate tax line at checkout for that item — matches what the customer sees). As of 2026-08, these items record their **full price as subtotal with $0 tax** everywhere: checkout draft math already excluded them from `taxableItemsCents`, and the post-payment inventory adjustment (`dbApplyPaidInventoryItemAdjustment` in `orders/db2.ts`) no longer backs an implicit tax portion out of the sold amount either — it did between 2026-06-23 and 2026-08-19 (commit `be4233c5`), which silently split e.g. a $100 tax-included sale into ~$92 subtotal + ~$8 tax on the order record and the inventory item's `sold_amount`, even though the customer was only ever charged $100. Affected historical orders (10 orders, 11 line items, all in that window) were corrected back to full price/$0 tax, including `ccg_inventory_items.sold_amount` (so Value Reports/Shop Statistics reflect it too) — each has an `order_events` row of type `sales_tax_included_amount_corrected` documenting the reversal.
- Cash checkout orders were never affected by the above bug — `dbMarkManualCheckoutOrderPaid` always recorded tax-included items at full price.
...
- `POST /api/admin-v2/orders/reconcile-stripe-checkout`
  ...
  - No Admin V2 UI for this anymore (removed after the one-time cleanup it was built for); call it directly (`POST`, admin-authenticated) if a similar gap is ever suspected again
