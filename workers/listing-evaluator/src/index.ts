...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (path.endsWith('/fb-add') && path.startsWith('/api/inventory/') && request.method === 'POST') {
      const response = await handleInventoryFbAdd(request, path, env);
      return withCors(response, request, env);
    }
    ...
    if (path.endsWith('/fb-include') && path.startsWith('/api/inventory/') && request.method === 'POST') {
      const response = await handleInventoryFbInclude(request, path, env);
      return withCors(response, request, env);
    }
