...
function normalizeMfrOrderFileName(fileName: string): string {
...
async function handleAdminV2MfrOrders(env: Env): Promise<Response> {
  const result = await env.DB.prepare(
    `SELECT
       o.id,
       o.po_number,
...
async function handleAdminV2MfrOrderCreate(request: Request, env: Env): Promise<Response> {
  let body: Record<string, unknown> = {};
  try {
    body = await request.json();
  } catch {
...
async function handleAdminV2MfrOrderFiles(orderId: number, env: Env): Promise<Response> {
...
async function handleAdminV2MfrOrderFileUpload(request: Request, orderId: number, env: Env): Promise<Response> {
  if (!env.CUSTOM_ITEMS_BUCKET) {
...
async function handleAdminV2MfrOrderFileOpen(fileId: string, env: Env): Promise<Response> {
  if (!env.CUSTOM_ITEMS_BUCKET) {
...
async function handleAdminV2MfrOrderFileDelete(fileId: string, env: Env): Promise<Response> {
  const row = await dbGetMfrOrderFile(fileId, env);
  if (!row) return jsonResponse({ message: 'File not found.' }, 404);
  const key = normalizeText(row.r2_key, '');
...
function mapMfrOrderRow(row: Record<string, unknown>): Record<string, unknown> {
  return {
    id: Number(row.id || 0),
...
function mapMfrOrderFileRow(row: Record<string, unknown>): Record<string, unknown> {
...
async function dbMfrOrderExists(orderId: number, env: Env): Promise<boolean> {
...
async function dbListMfrOrderFiles(orderId: number, env: Env): Promise<Record<string, unknown>[]> {
  const result = await env.DB.prepare(
    `SELECT *
     FROM mfr_order_file
     WHERE mfr_order_id = ?
...
async function dbGetMfrOrderFile(fileId: string, env: Env): Promise<Record<string, unknown> | null> {
...
async function handleAdminV2InventoryBackfillBarcodes(_env: Env): Promise<Response> {
  return jsonResponse({
