import { initialConfig } from 'config';
import { users } from 'data/users';
import { CommentItemType, ContentItem } from 'types/content';

const content = (index: number) => `${initialConfig.assetsDir}/images/content/list/${index}.webp`;
const localContent = (name: string) => `${import.meta.env.BASE_URL}images/content/${name}`;

export const contentList: ContentItem[] = [
  {
    id: 1,
    title: 'Guitar Brand Serial Number Decoders',
    category: 'Production Year & More',
    key: 'time-management',
    author: 'Always being updated',
    date: '2024-12-22',
    requiredTime: 'Over 25 major brands',
    image: localContent('serial-decoders.png'),
    type: 'blog',
    variant: 'featured',
  },
  {
    id: 2,
    title: 'Repair, Maintenance, & Gear Demo Videos',
    category: 'Video Content',
    key: 'technology',
    author: 'New Content Bi-Weekly',
    date: '2024-12-22',
    image: localContent('repair-demo-videos.png'),
    type: 'video',
    requiredTime: 'Average 5m each',
    variant: 'highlight',
  },
  {
    id: 3,
    title: 'Guitars and Gear for Sale',
    category: 'Shopping',
    key: 'travel',
    author: 'Financing options available',
    date: '2024-12-22',
    image: localContent('for-sale.png'),
    type: 'podcast',
    requiredTime: 'Guitars, Amps, Pedals, and much more',
    variant: 'regular',
  },
  {
    id: 4,
    title: 'Simple Daily Habits to Stay Healthy and Energized',
    category: 'Health',
    key: 'health',
    author: 'Michael Johnson',
    date: '2024-12-22',
    image: content(4),
    requiredTime: '11 min read',
    type: 'blog',
    variant: 'regular',
  },
  {
    id: 5,
    title: "How Hollywood's Biggest Blockbusters Are Made",
    category: 'Culture',
    key: 'culture',
    author: 'Rachel Collins',
    date: '2024-12-22',
    image: content(5),
    requiredTime: '12 min read',
    type: 'blog',
    variant: 'regular',
  },
  {
    id: 6,
    title: 'Into the Wild: Stories of Conservation and Adventure',
    category: 'Wildlife',
    key: 'wildlife',
    author: 'Jessica Lane',
    date: '2024-12-22',
    image: content(6),
    requiredTime: '30 min listen',
    type: 'podcast',
    variant: 'regular',
  },
  {
    id: 7,
    title: 'Pro Gamer Secrets: How to Level Up Your Skills Fast',
    category: 'Gaming',
    key: 'gaming',
    author: 'Ethan Miles',
    date: '2024-12-21',
    image: content(7),
    type: 'video',
    requiredTime: '1 hrs watch',
    variant: 'highlight',
  },
  {
    id: 8,
    title: 'NASA’s Bold Missions: Exploring the Future of Space Exploration',
    category: 'Astronomy',
    key: 'astronomy',
    author: 'AstroGeek TV',
    date: '2024-12-22',
    image: content(8),
    type: 'video',
    requiredTime: '1 hrs watch',
    variant: 'highlight',
  },
  {
    id: 9,
    title: '10 Fitness Myths You Need to Stop Believing Right Now',
    category: 'Fitness',
    key: 'fitness',
    author: 'James Rowe',
    date: '2024-12-22',
    requiredTime: '7 min read',
    image: content(9),
    type: 'blog',
    variant: 'highlight',
  },
  {
    id: 10,
    title: 'The Startup Journey: Stories of Success and Failure',
    category: 'Entrepreneurship',
    key: 'entrepreneurship',
    author: 'Zelda Daniels',
    date: '2024-12-22',
    requiredTime: '17 min listen',
    image: content(10),
    type: 'podcast',
    variant: 'regular',
  },
  {
    id: 11,
    title: 'How to Improve Communication Skills in Any Relationship',
    category: 'Relationships',
    key: 'relationships',
    author: 'Love & Connection Hub',
    date: '2024-12-22',
    requiredTime: '1.25 hrs watch',
    image: content(11),
    type: 'video',
    variant: 'highlight',
  },
  {
    id: 12,
    title: 'Soundscapes: Exploring Music, Art, and Culture',
    category: 'Music',
    key: 'music',
    author: 'Mark Stevens',
    date: '2024-12-22',
    requiredTime: '6 min read',
    image: content(12),
    type: 'blog',
    variant: 'regular',
  },
  {
    id: 13,
    title: 'The Best 10 Renowned Online Learning Platforms for Skill Development',
    category: 'Education',
    key: 'education',
    author: 'Sophie Clarke',
    date: '2024-12-22',
    requiredTime: '5 min watch',
    image: content(13),
    type: 'video',
    variant: 'highlight',
  },
  {
    id: 14,
    title: 'Reel Talk: Behind the Scenes of Film & TV',
    category: 'Entertainment',
    key: 'entertainment',
    author: 'Gary Johnson',
    date: '2024-12-22',
    requiredTime: '2 hrs read',
    image: content(14),
    type: 'blog',
    variant: 'regular',
  },
  {
    id: 15,
    title: 'How to Save, Invest, and Achieve Financial Freedom',
    category: 'Finance',
    key: 'finance',
    author: 'Gary Muytenchek',
    date: '2024-12-22',
    requiredTime: '11 min read',
    image: content(15),
    type: 'blog',
    variant: 'regular',
  },
  {
    id: 16,
    title: 'Top 10 Must-Visit Destinations for an Unforgettable Trip',
    category: 'Travel',
    key: 'travel',
    author: 'Alex Vandruck',
    date: '2024-12-22',
    requiredTime: '11 min read',
    image: content(16),
    type: 'blog',
    variant: 'regular',
  },
  {
    id: 17,
    title: 'Time Travelers: Uncovering the Past, Exploring the Future',
    category: 'History',
    key: 'history',
    author: 'Dr. Alex Monroe',
    date: '2024-12-21',
    requiredTime: '15 min read',
    image: content(17),
    type: 'blog',
    variant: 'regular',
  },
  {
    id: 18,
    title: 'Digital Defense: Cybersecurity Trends to Watch',
    category: 'Cybersecurity',
    key: 'cybersecurity',
    author: 'Sarah Connor',
    date: '2024-12-21',
    requiredTime: '45 min listen',
    image: content(18),
    type: 'podcast',
    variant: 'regular',
  },
  {
    id: 19,
    title: 'The Most Exciting Scientific Discoveries of 2024',
    category: 'Science',
    key: 'science',
    author: 'Dr. Nathan Clay',
    date: '2024-12-20',
    requiredTime: '12 min read',
    image: content(19),
    type: 'blog',
    variant: 'regular',
  },
];

export const comments: CommentItemType[] = [
  {
    id: 1,
    author: {
      ...users[13],
      name: 'Uncle Owen',
    },
    createdAt: '2025-09-14T15:14:00',
    message: {
      text: 'Modern architecture is truly redefining our world! The blend of aesthetics and functionality is mind-blowing.',
    },
    engagement: {
      likes: 2000,
      comments: 34,
    },
    replies: [],
  },
  {
    id: 2,
    author: users[3],
    createdAt: '2025-09-14T14:28:00',
    message: {
      text: "Loved the futuristic designs in this video! It's amazing to see how innovation is shaping our cities.",
    },
    engagement: {
      likes: 14200,
      comments: 134,
    },
    replies: [
      {
        id: 3,
        author: users[8],
        createdAt: '2025-09-14T14:50:00',
        message: {
          text: 'The focus on sustainability in architecture is inspiring! Hope to see more eco-friendly designs in the future.',
        },
        engagement: {
          likes: 11,
          comments: 3,
        },
        replies: [],
      },
      {
        id: 4,
        author: {
          ...users[13],
          name: 'Uncle Owen',
        },
        createdAt: '2025-09-14T15:15:00',
        message: {
          text: "These structures are truly ahead of their time! Can't wait to see what the next decade brings for modern architecture.",
        },
        engagement: {
          likes: 32,
          comments: 19,
        },
        replies: [],
      },
      {
        id: 5,
        author: users[5],
        createdAt: '2025-09-14T15:20:00',
        message: {
          text: 'Incredible how modern architecture blends technology and creativity! AI and 3D printing are changing everything.',
        },
        engagement: {
          likes: 22,
          comments: 13,
        },
        replies: [],
      },
    ],
  },
];
